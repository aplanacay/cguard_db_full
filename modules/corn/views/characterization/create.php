...
<div class="characterization-base-search container-fluid">
    ...
    $form = ActiveForm::begin([
                ...
                // 'method' => 'post',
                 'action' => ['characterization/create'],
                // 'options' => ['method' => 'post'],
                'layout' => 'horizontal',
                'fieldConfig' => [
                    'template' => "{label}\n{beginWrapper}\n{input}\n{hint}\n{error}\n{endWrapper}",
...
//                // if ($dataProvider->pagination->totalCount !== '0') {
                $phl_no_str = "'(^[0-9]+)'";
                $phl_no_str2 = "'([^0-9_].*$)'";
                echo $form->field($model, 'germplasm_id')->widget(Select2::classname(), [
                    'data' => \yii\helpers\ArrayHelper::map(\app\modules\corn\models\Germplasm::find()->groupBy('phl_no,id')->orderBy("(substring(phl_no, {$phl_no_str}))::int, substring(phl_no, {$phl_no_str2})")->all(), 'id', 'phl_no'),
                    'options' => ['placeholder' => 'Select PHL NO...'],
                    'pluginOptions' => [
                        'allowClear' => true
                    ]]
                )->label('PHL No.');;
//                echo $form->field($model->germplasm, 'variety_name')->textInput();
//
//
//                echo '</div><div class="col-md-4">';
//                echo $form->field($model->germplasm, 'gb_no')->textInput();
//
//                echo $form->field($model->germplasm, 'scientific_name', ['template' => "{label}<div class='col-sm-6'><i>{input}</i>{hint}{error}</div>",
//                ])->textInput();
//                echo '</div><div class="col-md-4">';
//                echo $form->field($model->germplasm, 'old_acc_no')->textInput();
//                echo $form->field($model->germplasm, 'other_number')->textInput();
//                echo '</div>';
                // }
                // ActiveForm::end();
                // echo '</div>';
